import os
os.environ["CUDA_VISIBLE_DEVICES"] = "4" # set gpu number
import numpy as np
...
audio_dir = "/data/yss/dataset/AVE_Dataset/AVE/compress_0330/audio_train_256" # .wav audio files
lis = os.listdir(audio_dir)
len_data = len(lis)
audio_features = np.zeros([len_data, 10, 128])
dir_fc = "/data/yss/dataset/AVE_compress/vggish_test"
...
for n in range(len_data):
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
        ...
        outfile = os.path.join(dir_fc, audio_name + '.npy')
...
with h5py.File('./feats/audio_embedding.h5', 'w') as hf:
